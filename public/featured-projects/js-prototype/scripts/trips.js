...
function renderTripCards(tripsList, tours) {
    ...
    if (filteredTourData.length === 0) {
        ...
        const tripCards = filteredTourData
            .map((tour) => {
                const tripData = tripsList.find(
                    (trip) => trip.tourId === tour.id
                );
                return `
...
export { getTripsListView, deleteTrip, renderTripCards, getConfirmedToursList };
